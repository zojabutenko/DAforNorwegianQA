...
def normalize_text(text):
    ...
    text = re.sub(r"[^\w\sæøå]", "", text)  # Remove punctuation (but keep Norwegian chars)
...
def squad_convert_example_to_features(
    ...
    if is_training and not example.is_impossible:
        ...
        # logger.info(f'actual_text: {actual_text}')
        actual_text = normalize_text(actual_text)
        # logger.info(f'actual_text after normalization: {actual_text}')
        cleaned_answer_text = " ".join(whitespace_tokenize(example.answer_text)).lower()
        cleaned_answer_text = normalize_text(cleaned_answer_text)
...
class SquadProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, filename=None):
        ...
        with open(
            ...
            input_data = json.load(reader, strict=False)["data"]
